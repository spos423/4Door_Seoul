...
<body>
	...
	<main class="container-fluid px-0 mb-5" id="main">
		...
		<h5 class="present_location"><img alt="" src="/resources/img/home.png" class="where"> > 여행정보 > 축제&행사 > 게시글 수정</h5>
	  	<br>
			  	...
			  	<form action="t_update_proc.traveldo" method="post" name="boardForm" enctype="multipart/form-data">
					...
					<div class="form-group">
				     ...
				     <div class="form-group">
				      	...
				      	<input type="text" class="form-control" id="url_info" placeholder="웹사이트 입력" name="uri" value="${uri}">
				    </div>
				  
			   		<div class="form-group">
						<label for="start_event" class="label">* 이용 기간 및 시간 (시작)</label><br>
						<input type="datetime-local" class="form-control" id="start_datetime" name="start_datetime" value="${time_s}">
					</div>
					<div class="form-group">	
						<label for="end_event" class="label">* 이용 기간 및 시간 (종료)</label><br>
						<input type="datetime-local" class="form-control" id="end_datetime" name="end_datetime" value="${time_e}">
					</div>
				    ...
				    <div class="form-group">
				    	<label for="fee_info" class="label">* 이용 요금</label>
				       	<textarea class="form-control fee" rows="30" cols="10" id="fee_info"
				      		      placeholder="이용 요금 입력" name="price">${price}</textarea>
				    </div>
					...
					<input type="button" class="btn" value="게시글 수정" onclick="u_formCheck0()" />
			    </form>
			</div>
		</div>
