...
<body>
	...
	<main class="container-fluid px-0 mb-5" id="main">
		<h5 class="present_location"><img alt="" src="/resources/img/home.png" class="where"> > 여행정보 > 볼거리 > 게시글 수정</h5>
	  	<br>
			  	...
			  	<form action="t_update_proc.traveldo" method="post" name="boardForm" enctype="multipart/form-data">
					...
					<div class="form-group">
				        ...
				        <select name="ct_d" class="form-control" id="ct_d">
				        	<c:if test="${ct_d==3}">
					        	<option value="3" selected>명소</option>
					        	<option value="4">자연&경관</option>
					        	<option value="5">엔터테인먼트</option>
				        	</c:if>
				        	<c:if test="${ct_d==4}">
					        	<option value="3">명소</option>
					        	<option value="4" selected>자연&경관</option>
					        	<option value="5">엔터테인먼트</option>
				        	</c:if>
				        	<c:if test="${ct_d==5}">
					        	<option value="3">명소</option>
					        	<option value="4">자연&경관</option>
					        	<option value="5" selected>엔터테인먼트</option>
				        	</c:if>
				        </select>
				    </div>
					...
					<input type="button" class="btn" value="게시글 수정" onclick="u_formCheck1()" />
			    </form>
			</div>
		</div>
